...
/* --- Composants --- */
  ...
  /* --- Grille de pads --- */
  const renderPadGrid = ()=>{
    if(lanes<=8) return <div style={{display:"flex",width:isPhone?"85vw":"95vw",maxWidth:isPhone?"90vw":560,margin:"0 auto"}}>{[...Array(lanes)].map((_,i)=><Pad key={i} i={i}/>)}</div>;
    const cols = (lanes===10 || lanes===20) ? 5 : 10;
    return <div style={{ display:"grid", gridTemplateColumns:`repeat(${cols}, 1fr)`, gap: isPhone ? 4 : (lanes===20 ? 12 : 20), width: isPhone?"85vw": (lanes===20 ? "min(85vw,540px)" : "90vw"), margin:"0 auto", maxWidth:620 }}>{[...Array(lanes)].map((_,i)=><Pad key={i} i={i}/>)}</div>;
  };
...
const hud = { color:"#fff", marginTop:isPhone?60:24, textAlign:"center", minHeight:74 };
